...
class DESolver:
    ...
    def GenerateOrderTestGrid(self,NRuns,t_min,t_max,dt_min,dt_max):
        ...
        NSteps_tab=np.asarray(np.logspace(np.log10(NSteps_min),np.log10(NSteps_max),NRuns)).astype('int')
    ...
    def GLEE_Starting_Procedure(self):
        ...
        self._r=METHOD['r']
        self._solution_aux=np.zeros((self._n,self._r-1),dtype=self._function_context['data-type'])
    ...
    def step(self):
        ...
        if(self._info==1):
            print(' ---------- step {:03d}: t={:05g} {:}:{:}'.format(self._step,t,METHOD['type'],self._current_method))
        ...
        if(METHOD['type']=='GLEE'):# or METHOD['type']=='GLEE-EIMEX'):
            ...
            if(self._solution_aux is None):
                ...
                q_in=np.zeros((self._n,self._r),dtype=self._function_context['data-type'])
            ...
            if(self._info==2):
                print(' ---------- step {:d}: t={:g}, u_in = u[{:d}], u_out=[{:d}],'.format(self._step,t,self._step,self._step+1))
            Ystage=np.zeros((n,s),dtype=self._function_context['data-type'])        
            Kstage=np.zeros((n,s),dtype=self._function_context['data-type'])
            ...
            for istage in range(s):
                ...
                for jstage in range(istage): # should be istage
                    Ystage[0:n,istage]=Ystage[0:n,istage]+dt*A[istage,jstage]*Kstage[0:n,jstage]
                for jstage in range(r):
                    Ystage[0:n,istage]=Ystage[0:n,istage]+U[istage,jstage]*q_in[0:n,jstage]
                ...
                if(A[istage,istage]!=0.):
                    ...
                    def IFunction_Jac(X,*data):
                        ...
                        t_stage_in=data[1]
                        Res_in=data[2]                        
                        f,Jac=self._rhs(t_stage_in,X,self._function_context)
                        J=np.eye(Jac.shape[0])-alpha*Jac                        
                        return J

                    params = (dt*A[istage,istage], t_stage, Ystage[0:n,istage])
                    Ystage[0:n,istage] = fsolve(func=IFunction, fprime=IFunction_Jac, x0=q_in[0:n,0], args=params)
                
                Kstage[0:n,istage],_=self._rhs(t_stage,Ystage[0:n,istage],self._function_context)
                
                
                
                
            q_out=np.zeros(q_in.shape,dtype=self._function_context['data-type'])
            for rstage in range(r):
                for jstage in range(s):
                    q_out[0:n,rstage]=q_out[0:n,rstage]+dt*B[rstage,jstage]*Kstage[0:n,jstage]
                for jstage in range(r):
                    q_out[0:n,rstage]=q_out[0:n,rstage]+V[rstage,jstage]*q_in[0:n,jstage]


            u_out=np.zeros(self._solution.shape,dtype=self._function_context['data-type'])
            u_out[0:n]=q_out[0:n,0]
            for i in range(self._r-1):
                self._solution_aux[0:n,i]=q_out[0:n,i+1]
                        
                
            self._u_high        = (1./(1.-gamma))*q_out[0:n,1]-(gamma/(1-gamma))*q_out[0:n,0]
            self._epsilon       = (1./(1.-gamma))*(q_out[0:n,1]-q_out[0:n,0])
            self._epsilon_local = (1./(1.-gamma))*(q_out[0:n,1]-q_out[0:n,0])-(1/(1-gamma))*(q_out[0:n,1]-q_out[0:n,0])
            ...
            if(self._rhs_e is None or self._rhs_i is None):
                raise NameError('rhs_e, rhs_i need to be defined in order to use GLEE-EIMEX integrators.')
            #print('Solutions: {:} {:}'.format(q_in[0,0],q_in[0,1]))
            ...
            Ystage=np.zeros((n,s),dtype=self._function_context['data-type'])        
            KstageE=np.zeros((n,s),dtype=self._function_context['data-type'])                    
            KstageI=np.zeros((n,s),dtype=self._function_context['data-type'])
            ...
            for istage in range(s):
                Ystage[0:n,istage]=0.
                for jstage in range(s):
                    Ystage[0:n,istage]=Ystage[0:n,istage]+dt*A[istage,jstage]*KstageE[0:n,jstage]+dt*At[istage,jstage]*KstageI[0:n,jstage]
                for jstage in range(r):
                    Ystage[0:n,istage]=Ystage[0:n,istage]+U[istage,jstage]*q_in[0:n,jstage]
                ...
                if(At[istage,istage]!=0.):
                    def IMEX_Function(X,*data):
                        ...
                        t_stage_in=data[1]
                        Res_in=data[2]
                        f,_=self._rhs_i(t_stage_in,X,self._function_context)
                        F=X-alpha*f-Res_in   
                        return F
                    ...
                    def IMEX_Function_Jac(X,*data):
                        ...
                        t_stage_in=data[1]
                        Res_in=data[2]                        
                        f,Jac=self._rhs_i(t_stage_in,X,self._function_context)
                        J=np.eye(Jac.shape[0])-alpha*Jac                        
                        return J

                    params = (dt*At[istage,istage], t_stage, Ystage[0:n,istage])
                    #x,_=IMEX_Function(X,*params)
                    Ystage[0:n,istage] = fsolve(func=IMEX_Function, fprime=IMEX_Function_Jac, x0=q_in[0:n,0], args=params)
                
                
                
                KstageE[0:n,istage],_=self._rhs_e(t_stage,Ystage[0:n,istage],self._function_context)
                KstageI[0:n,istage],_=self._rhs_i(t_stage,Ystage[0:n,istage],self._function_context)   
                
            q_out=np.zeros(q_in.shape,dtype=self._function_context['data-type'])
            for rstage in range(r):
                for jstage in range(s):
                    q_out[0:n,rstage]=q_out[0:n,rstage]+dt*B[rstage,jstage]*KstageE[0:n,jstage]
                for jstage in range(r):
                    q_out[0:n,rstage]=q_out[0:n,rstage]+V[rstage,jstage]*q_in[0:n,jstage]
            ...
            for rstage in range(r):
                for jstage in range(s):
                    q_out[0:n,rstage]=q_out[0:n,rstage]+dt*bt[jstage]*KstageI[0:n,jstage]
            ...
            if(self._rhs==None or self._rhs_gint==None):
                raise NameError('rhs_e, rhs_gint need to be defined in order to use RK-IDE integrators.')
            if(self._info==2):
                    print(' ---------- step {:d}: t={:g}, u_in = u[{:d}], u_out=[{:d}],'.format(self._step,t,self._step,self._step+1))
            ...
            Kstage[0:n,0],_=self._rhs(t_stage,Ystage[0:n,0],self._function_context)
            
            if(self._step>=1):
                if(self._info==2):
                    print('Step {:d}: In(1)={:g}, In(2)={:g}'.format(self._step,self._In[0],self._In[1]))
                Kstage[0:n,0]=Kstage[0:n,0]+0.5*dt*(self._In+self._rhs_gint(t,t,u_in,self._function_context))
                if(self._info==2):
                    print('Step {:d}: K1=K1+0.5*dt*(In+g(t={:g},t={:g},u_in))'.format(self._step,t,t))
            ...
            if(self._info==2):
                    print('Step {:d}: In=g(t+dt={:g},t0={:g},u0), u[{:d}](1)= {:g} u[{:d}](2)={:g}'.format(self._step,t+dt,self._t_start,0, self._u_trajectory[0,0], 0,self._u_trajectory[1,0]))
            self._In=self._rhs_gint(t+dt,self._t_start, self._u_trajectory[0:n,0],self._function_context)
            
           
            for i in range(1,self._step):
                if(self._info==2):
                    print('Step {:d}: In=In+2*g(t+dt={:g},t={:g},u[{:d}]), u[{:d}](1)= {:g} u[{:d}](2)={:g}'.format(self._step,t+dt,self._t_trajectory[i],i,i,self._u_trajectory[0,i],i,self._u_trajectory[1,i]))
                self._In=self._In+2.*self._rhs_gint(t+dt,self._t_trajectory[i], self._u_trajectory[0:n,i],self._function_context)


            if(self._step>=1):
                if(self._info==2):
                    print('Step {:d}: In=In+2*g(t+dt={:g},t={:g},u_in)'.format(self._step,t+dt,t))
                self._In=self._In+2.*self._rhs_gint(t+dt,t,u_in,self._function_context)
                
            Kstage[0:n,1],_=self._rhs(t_stage,Ystage[0:n,1],self._function_context)
           
            if(self._info==2):
                print('Step {:d}: K2=f(t,Y2)+0.5*dt*(In+g(t={:g},t={:g},Y2))'.format(self._step,t_stage,t_stage))
            Kstage[0:n,1]=Kstage[0:n,1]+0.5*dt*(self._In+self._rhs_gint(t_stage,t_stage,Ystage[0:n,1],self._function_context))
            if(self._info==2):
                print('Step {:d}: In(1)= {:g}, In(2)={:g}'.format(self._step,self._In[0],self._In[1]))
            u_out=u_in.copy()
            ...
            if(approx_exp is False):
                ...
                if(p==2):
                    ...
                    _,Htdt=self._rhs(t+dt,u_full_approx,self._function_context) 
            ...
            for istage in range(s):
                ...
                if(At[istage,istage]==0):
                    ...
                    if(u_in.dtype==np.dtype('complex128')):
                        outp = fsolve(func=IM_Function, x0=decomplexify(u_in), args=params, xtol=1.0e-12)
                        Ystage[0:n,istage] = complexify(outp)
                    else:
                        Ystage[0:n,istage] = fsolve(func=IM_Function, fprime=IM_Function_Jac, x0=u_in, args=params, xtol=1.0e-12)
            ...
            if(self._rhs_e==None or self._rhs_i == None):
                raise NameError('rhs_e, rhs_i need to be defined in order to use ARKq integrators.')
            ...
            for istage in range(s):
                ...
                Res=u_in
                for jstage in range(istage+1):
                    Res=Res+dt*A[istage,jstage]*KstageE[0:n,jstage]+dt*At[istage,jstage]*KstageI[0:n,jstage]
                if(At[istage,istage]==0):
                    Ystage[0:n,istage]=Res
                else:
                    def IMEX_Function(X,*data):
                    ...
                    #x,_=IMEX_Function(X,*params)
                    Ystage[0:n,istage] = fsolve(func=IMEX_Function, fprime=IMEX_Function_Jac, x0=u_in, args=params)
        ...
        if(self._keep_history==True):
            self._t_trajectory=np.hstack((self._t_trajectory,np.asarray(self._t)))
            self._u_trajectory=np.hstack((self._u_trajectory,np.reshape(self._solution,(n,1))))
            if(self._has_global_error==True):
                self._epsilon_trajectory=np.hstack((self._epsilon_trajectory,np.reshape(self._epsilon,(n,1))))
                self._epsilon_local_trajectory=np.hstack((self._epsilon_local_trajectory,np.reshape(self._epsilon_local,(n,1))))
                self._u_high_trajectory=np.hstack((self._u_high_trajectory,np.reshape(self._u_high,(n,1))))
    ...
    def set_initial_solution(self, U0):
        ...
        if(self._keep_history==True):
            self._u_trajectory=np.zeros((self._n,1),dtype=self._function_context['data-type'])
